...
//    }
	...
	private int getMarginHorizontal(int i) {
		int result = 0;
		int mid = numPages % 2;
		if (mid == 0) {
			result = getWidth() / 2 + (i - mid) * 2 * getCircleRadius()
					+ (i - mid + 1 / 2) * pageSize;
		} else {
			result = getWidth() / 2 + (2 * (i - mid) + 1) * getCircleRadius()
					+ (i - mid) * pageSize;
		}
		return result;
	}
    	...
    	if(getNumPages()>1){
	    	do{
	    		for(int i=0;i<getNumPages();i++){
	    	    	  canvas.drawCircle(getMarginHorizontal(i), getMarginVertral(),getCircleRadius(), barPaint);
	    	    }	
	    	}while(false);
	...
	//      canvas.drawBitmap(bmp, mMatrix, highlightPaint);
           canvas.drawCircle(getMarginHorizontal(currentPage), getMarginVertral(),getCircleRadius(), highlightPaint);
    	}
    }
}
